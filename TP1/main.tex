...
\section{Pasabajos}
    ...
    \begin{equation}
\frac{1}{\frac{S}{\frac{1}{R \cdot C}}+1}
\label{eq:pasabajos}
\end{equation}
    ...
    \subsubsection {}%b
	Para este circuito se utilizaron una $R = 4,7 k\Omega \pm 5 \%$ y un $C = 3,3 nF \pm 10\%$. \par
...
\subsubsection {}%c
	...
	Puesto a que el circuito es una malla, todos sus componentes poseen la misma $I$. Luego la la diferencia de fase está dada por $\angle V_c - \angle I = -90$. (Observar deducción \ref{subsec:desfase}). \par 
...
\end{tikzpicture}
\caption{Diagrama fasorial representativo (no a escala entre magnitudes de distinta unidad).}
\label{fig:dfasorial}
\end{figure}
...
Para esta parte, se realizo el diagrama del bode del circuito, combinando todos los casos posibles, considerando los valores mínimos, nominales y máximos de los componentes. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\textwidth]{1.1d}
    \caption{Diagrama de bode posibles tomando valor máximo, nominal y mínimo de los componentes}
    \label{fig:bode1.1d}
\end{figure}

Como observación de la figura \ref{fig:bode1.1d}, la curva que baja primero es aquella en la cual el capacitor y la resistencia tienen el valor máximo, ya que consiguen un $f_{0min}=8884 Hz$. Luego la curva que más tarde baja es aquella que tiene los componentes con los valores mínimos, ya que consigue la frecuencia de corte más alta de todas, la cual es $f_{0max} = 12000 Hz$.


\subsubsection{}%e

De la ecuación \ref{eq:pasabajos}, se despejaron los siguientes valores:
...
Para esta parte, se simularon los tres casos (Observar imagenes \ref{fig:20f0}, \ref{fig:f0} y \ref{fig:f0sobre20}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{1.1f(f0.20)}
    \caption{Gráfico con $f=20 \cdot f_0$.}
    \label{fig:20f0}
\end{figure}
    \end{minipage}
    \hfill
    ...
    \begin{minipage}[b]{0.45\textwidth}
        ...
        \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{1.1f(f0)}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{1.1f(f0sobre20)}
...
\begin{align*}
    ...
    \implies V_{out}&= V_{in} \cdot \frac{1}{1+sCR}\\
    Pero\ como &\ \frac{f}{f_0}>> 1\ ,desprecio +1\\
    \implies V_{out}&= V_{in} \cdot \frac{1}{sCR}\\
    Luego\ antitransfo&rmando\ y\ recordando\ la\ propiedad\\ y(t) &= h(t) * x(t)\\
    y(t) &= \frac{1}{RC} \cdot \int_{}^{}x(t)\\
\end{align*}
...
\section{Pasaaltos}
    \subsection{Parte teórica}
        % a) Transferencia y frecuencia de corte
        Para el análisis teórico se consideró el circuito RC pasaaltos incluyendo únicamente la resistencia interna del generador, modelada como una resistencia en serie de $R_{gen} = 50\,\Omega$.

Se decidió despreciar el efecto del osciloscopio (tanto su resistencia de entrada de 1 M$\Omega$ como la capacitancia parásita asociada a la punta), ya que para el rango de frecuencias de interés (menores a 500 kHz) su influencia resulta despreciable frente a las impedancias del circuito.

Bajo estas condiciones, el circuito equivalente utilizado es el siguiente:
...
to[R, l=$50\,\Omega$] (2,3)
to[C, l=$3.4\,\text{nF}$] (4,3)
to[R, l_=$4.6\,\text{k}\Omega$] (4,0)
-- (0,0);
...
% flecha vout (corrida un poco a la derecha)
\draw[->, >=stealth] (5.3,0.5) -- (5.3,2.5)
node[midway,right] {$v_{out}$};

\end{circuitikz}
\shorthandon{>}
\caption{Circuito RC pasaaltos considerando la resistencia interna del generador}
\label{fig:hp_rgen}
\end{figure}

A partir del circuito de la Fig.~\ref{fig:hp_rgen}, la función de transferencia puede obtenerse aplicando divisor de impedancias. Considerando la impedancia del capacitor como:

\[
Z_C = \frac{1}{sC}
\]

la salida se toma sobre la resistencia \(R\), por lo que:

\begin{equation}
H(s) = \frac{V_{out}}{V_{in}} = \frac{R}{R + R_{gen} + \frac{1}{sC}}
\end{equation}

Multiplicando numerador y denominador por \(sC\), se obtiene una expresión más conveniente:

\begin{equation}
H(s) = \frac{sRC}{1 + sC(R + R_{gen})}
\end{equation}

Reemplazando ahora \(s = j 2\pi f\) y los valores del circuito (\(R = 4.6\,\text{k}\Omega\), \(R_{gen} = 50\,\Omega\), \(C = 3.4\,\text{nF}\)), resulta:

\begin{equation}
H(f) = \frac{j 2\pi f \cdot (4.6\times10^3)\cdot(3.4\times10^{-9})}{1 + j 2\pi f \cdot (3.4\times10^{-9}) \cdot (4.6\times10^3 + 50)}
\end{equation}

A partir de la función de transferencia obtenida, la frecuencia de corte puede determinarse de distintas maneras.

Por un lado, en el dominio de la frecuencia, la frecuencia de corte se define como aquella para la cual el módulo de la función de transferencia cae 3 dB respecto del valor máximo. En este caso, el módulo de la transferencia es:

\[
|H(j\omega)| = \left| \frac{j\omega RC}{1 + j\omega C(R + R_{gen})} \right|
\]

Calculando su módulo:

\[
|H(j\omega)| = \frac{\omega RC}{\sqrt{1 + \left[\omega C(R + R_{gen})\right]^2}}
\]

La frecuencia de corte se obtiene cuando:

\[
20\log_{10}(|H(j\omega)|) = -3\,\text{dB}
\]
...
Por otro lado, en el dominio del tiempo, esto mismo implica que la amplitud de la señal de salida es igual a la de entrada dividida por \(\sqrt{2}\). En nuestro caso, al excitar el circuito con una señal senoidal de entrada de \(10\,\text{V}_{pp}\), se espera que en la frecuencia de corte la salida sea aproximadamente:

\[
V_{out} \approx \frac{10}{\sqrt{2}} \approx 7.07\,\text{V}_{pp}
\]

Finalmente, despejando de la expresión del módulo, se obtiene que la frecuencia de corte está dada por:

\begin{equation}
f_0 = \frac{1}{2\pi C(R + R_{gen})}
\end{equation}

Reemplazando los valores del circuito (\(R = 4.6\,\text{k}\Omega\), \(R_{gen} = 50\,\Omega\), \(C = 3.4\,\text{nF}\)):

\[
f_0 = \frac{1}{2\pi \cdot 3.4\times10^{-9} \cdot (4.6\times10^3 + 50)}
\]

\[
f_0 \approx 10.1\,\text{kHz}
\]
    ...
    \subsection{Parte práctica}
        \subsubsection*{a) Diagrama de Bode}
            ...
            Sin embargo, como se puede observar en la figura \ref{fig:pasaaltosSimulado}, en este circuito esas no-idealidades afectan poco al comportamiento en frecuencia del mismo. Para las resistencias, es porque la resistencia de $1 M\Omega$ queda en paralelo a la de $4k7\Omega$, y se tiene que la resistencia equivalente vale
            \begin{equation*}
                \frac{1}{\frac{1}{4,7k}+\frac{1}{1M}}\approx\frac{1}{\frac{1}{4,7k}} = 4,7k
            \end{equation*}
            puesto que $4,7k \ll 1M$, por lo que la resistencia de $ 1 M\Omega$ resulta despreciable.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{item2_simulacion.png}
                \caption{Simulación del pasaaltos: ideal vs real}
                \label{fig:pasaaltosSimulado}
            \end{figure}

            Luego, para los capacitores, las impedancias de ambos para $0,1f_0$, $f_0$ y $10f_0$ quedan como en la tabla \ref{table:impedanciasCapacivas} a continuación, sabiendo que $|Z_C|=\frac{1}{\omega C}$.
            ...
            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{item2_bodeTeorico.png}
                \caption{Diagrama de BODE teórico del filtro pasa-altos con puntos medidos del módulo.}
                \label{fig:pasaaltosTeorico}
            \end{figure}

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.5\linewidth]{item2_bodeMedido.png}
                \caption{Diagrama de BODE medido}
        ...
        de corte (10k) el filtro pasa-altos se comporta como un derivador, puesto que la derivada de
        una recta es una constante. Algebraicamente, esto se puede notar porque en su función transferencia
        
        \begin{equation*}
            H(s)=\frac{sRC}{1+sRC}
        \end{equation*}

        si $ sRC \ll 1 \Rightarrow H(s) \simeq sRC $ y multiplicar por s la transformada de una señal es equivalente a derivar la señal.\\
        ...
        tiempo, lo que se ve en el osciloscopio es una curva muy parecida a una recta .
            ...
            \begin{figure}[H]
                ...
                \caption{Respuesta del filtro pasa-altos ante una señal triangular a distintas frecuencias.}
...
\section{Sincronización de instrumentos}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{item3_modoXY.png}
        \caption{Barrido de la respuesta en frecuencia del filtro pasa-bajos en modo XY.}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{item3_modoNormal.png}
        \caption{Barrido de la respuesta en frecuencia del filtro pasa-bajos en modo normal.}
