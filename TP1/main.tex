...
\end{align*}
    ...
    \subsection{Parte práctica}
                % a) Frecuencia de corte real y cálculo de C
                    ...
                    Se necesario considerar la resistencia total del circuito $R$,
                    ...
                    la capacitancia $C$ de la ecuación teórica $f_{corte}$ y reemplazando con los valores de $R$ y $f_c$ obtenidos:
                    \begin{equation}
                        C = \frac{1}{2\pi R f_c} = \frac{1}{2\pi (4650\ \Omega) (10,05\text{ kHz})}
                    \end{equation}
        
                    Se obtiene $C \approx 3,41\text{ nF}$.
                ...
                En base a los datos recolectados en la tabla \ref{tab:pasa_bajos_freq}, se graficó los puntos correspondientes sobre el diagrama de Bode teórico obtenido en el punto 1.1.d), ver en anexos.
...
\section{Pasaaltos}
    \subsection{Parte teórica}
        \subsubsection{Transferencia y frecuencia de corte}
            ...
            \label{eq: func. transf pasaaltos}
            \end{equation}
        ...
        \subsubsection{Frecuencia nominal y error}
            Considerando los valores nominales de los componentes, la frecuencia de corte del filtro pasaaltos puede aproximarse como:

            \[
            f_0 = \frac{1}{2\pi RC}
            \]

            Tomando \(R = 4.7\,k\Omega\) y \(C = 3.3\,nF\), se obtiene:

            \[
            f_0 \approx 10261 \, \text{Hz}
            \]

            Para estimar el error en la frecuencia de corte, se utiliza la expresión obtenida a partir de propagación de errores:

            \begin{equation}
            \Delta f = f_0 \sqrt{\left(\frac{\Delta R}{R}\right)^2 + \left(\frac{\Delta C}{C}\right)^2}
            \end{equation}

            Considerando una tolerancia del \(5\%\) en la resistencia y del \(10\%\) en el capacitor:

            \[
            \Delta f \approx 1147 \, \text{Hz}
            \]

            Por lo tanto, la frecuencia de corte nominal resulta:

            \[
            f_0 \approx (10.26 \pm 1.15)\, \text{kHz}
            \]

            Se denota que el error se ve dominado por la incertidumbre del capacitor.
    ...
    \subsubsection{Diagrama de Bode con tolerancias}
            ...
            \centering
                \begin{tikzpicture}
                    ...
                    \end{semilogxaxis}
                \end{tikzpicture}
            \caption{Diagrama de Bode del módulo del filtro pasaaltos con sus distintas variantes.}
            \end{figure}
            ...
            \centering
                \begin{tikzpicture}
                    ...
                    \end{semilogxaxis}
                \end{tikzpicture}
            \caption{Diagrama de fase del filtro pasaaltos con sus distintas variantes.}
            \end{figure}

    \subsubsection{Ganancia y fase en distintas frecuencias con una sinosoidal}
            Para el circuito pasaaltos, tanto la funcion transferencia como su modulo se denotan en las ecuaciones \ref{eq: func. transf pasaaltos} y \ref{eq: modulo pasaaltos}.

            Luego para la fase:

            \begin{equation}
                \angle H(j\omega)=\arctan\left(\frac{1}{\omega RC}\right)
            \label{eq: fase de pasaaltos}
            \end{equation}

            Evaluando en las frecuencias solicitadas:
            ...
            Para $f=\frac{f_0}{10}$:
                ...
                \[
                |H| \approx 0.0995 \quad \Rightarrow \quad 20\log_{10}|H| \approx -20\,\text{dB}
                \]

                \[
                \angle H \approx 84.3^\circ
                \]
            ...
            Para $f=f_0$:
                ...
                \[
                |H|=\frac{1}{\sqrt{2}} \approx 0.707 \quad \Rightarrow \quad -3\,\text{dB}
                \]
            ...
            Para $f=10f_0$:
                ...
                \[
                |H| \approx 0.995 \quad \Rightarrow \quad 0\,\text{dB}
                \]

                \[
                \angle H \approx 5.7^\circ
                \]

    \subsubsection{Simulacion de una netrada de onda triangular}
        ...
        \centering
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
        \caption{Respuesta del filtro pasa-altos para $f = 10 f_0$.}
        \end{figure}
        ...
        \centering
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
        \caption{Respuesta del filtro pasa-altos para $f = f_0$.}
        \end{figure}
        ...
        \centering
            \begin{tikzpicture}
                ...
                \end{axis}
            \end{tikzpicture}
        \caption{Respuesta del filtro pasa-altos para $f = \frac{f_0}{10}$.}
        \end{figure}
    ...
    \subsubsection{Análisis analítico para entrada triangular}
        ...
        En el dominio de Laplace, esta relación se transforma en:

        \begin{equation}
            V_{out}(s) = H(s)\,V_{in}(s)
        \end{equation}

        donde $H(s)$ es la función de transferencia del circuito, definida en la ecuación \ref{eq: func. transf pasaaltos}.

        \vspace{0.5cm}

        Para analizar la respuesta frente a una señal triangular, se puede modelar dicha señal como una sucesión de tramos lineales. En un intervalo particular, la señal puede escribirse como:

        \begin{equation}
            v_{in}(t) = k t
        \end{equation}

        donde $k$ representa la pendiente del tramo.
        ...
        Por otro lado, en el régimen en el cual el circuito se comporta como derivador (bajas frecuencias), se cumple que:
        ...
        \begin{equation}
            V_{out}(s) = H(s)\,V_{in}(s) \approx (sRC)\cdot \frac{k}{s^2}
        \end{equation}

        \begin{equation}
            V_{out}(s) \approx \frac{kRC}{s}
        \end{equation}
        ...
        Es decir, la salida resulta constante en dicho intervalo.

        \vspace{0.5cm}

        Repitiendo el mismo análisis para el tramo decreciente de la señal triangular (pendiente negativa), se obtiene:

        \begin{equation}
            v_{out}(t) = -kRC
        \end{equation}

        Por lo tanto, la salida del sistema es una señal que alterna entre dos niveles constantes, lo que corresponde a una onda cuadrada.
        ...
        \end{equation}
            ...
            lo cual demuestra que, en el régimen considerado, el filtro pasa-altos actúa como un derivador.
    %
    %
    %
    %
